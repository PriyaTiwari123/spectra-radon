...
export const getAIRecommendation = async (req, res) => {
  try {
    const { pressure } = fetchSensorData();
    ...
    const normalize = (t, p, h) => [
      (t - tempMean) / tempStd,
      (p - pressMean) / pressStd,
      (h - humMean) / humStd,
    ];

    const normalizedInputs = rawInputs.map(([t, p, h]) => normalize(t, p, h));
    ...
    const [normTemp, normPress, normHum] = normalize(temperature, pressure, humidity);
    const inputTensor = tf.tensor2d([[normTemp, normPress, normHum]]);
    const predictionTensor = model.predict(inputTensor);
    const predictedSpeed = (await predictionTensor.data())[0];

    console.log(
      `Predicted Speed: ${predictedSpeed.toFixed(2)} | Temp: ${temperature}°C | Pressure: ${pressure} | Humidity: ${humidity}%`
    );
